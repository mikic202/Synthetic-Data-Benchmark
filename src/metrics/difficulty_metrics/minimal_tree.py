...
def calculate_ralation_between_dree_depth_and_accuaracy(
    ...
    real_x: pd.DataFrame,
    real_y: list[int | float],
):
    accuracy_for_tree_depth = {}
    if len(np.unique(real_y)) > NUMBER_OF_UNIQUE_ELEMENTS_FOR_CLASIFICATION or len(
        np.unique(synthetic_y) > NUMBER_OF_UNIQUE_ELEMENTS_FOR_CLASIFICATION
    ):
